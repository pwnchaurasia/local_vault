...
async def create_text_content(
    ...
    try:
        ...
        content = Content(
            ...
            tags=serialize_tags(content_data.tags),
        ...
        logger.info(f"Text content created: {content.id} by device {current_device.device_name}")
        ...
        raise HTTPException(status_code=500, detail="Failed to create text content")
...
async def upload_file_content(
    ...
    """Upload file content with metadata"""
    ...
    try:
        ...
        logger.info(f"File content uploaded: {file.filename} ({file_size} bytes) by device {current_device.device_name}")
        ...
        response_data = ContentResponse(
            ...
            download_url=f"/content/download/{content.id}"
        )
...
async def list_content(
    ...
    query = db.query(Content).filter(Content.device_id == current_user.id)
    ...
    for content in contents:
        response_data = ContentResponse(
            ...
            file_size=content.file_size,
            mime_type=content.mime_type,
            download_url=f"/content/download/{content.id}" if content.content_type == ContentType.FILE else None
        )
...
async def update_content(
    ...
    """Update content metadata and text content"""
    ...
    if update_data.tags is not None:
        content.tags = serialize_tags(update_data.tags)
    
    if update_data.text_content is not None:
        if content.content_type != ContentType.TEXT:
            raise HTTPException(status_code=400, detail="Cannot update text content for file content")
        content.text_content = update_data.text_content
    
    db.commit()
    db.refresh(content)
    
    logger.info(f"Content updated: {content_id} by device {current_user.phone}")
...
async def delete_content(
    ...
    logger.info(f"Content deleted: {content_id} by device {current_user.phone_number}")
...
async def get_content_stats(
    ...
    """Get content statistics for the current device"""
    ...
    return {
        ...
        "device_name": current_user.phone_number
    }
